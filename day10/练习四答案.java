...
注：按Java规范书写程序代码，如果你认为程序有错，请指出，并说明程序错误原因。
...
编译失败：因为A接口中并未定义func方法。该方法为子类特有。
...
class Demo extends Fu
{
	public static void main(String[] args)
	{
		...
		Fu f=new Demo();
		Demo d=new Demo();
		for(f.show('A');f.show('B')&&i<2;f.show('C'))//f.show('A')运行的是子类的。
		{
			i++;
			d.show('D');
		}
...
内部类中定义了静态成员，该内部类必须被静态修饰。

-----------------------------------
-----------------------------------
9.
选择题，写出错误答案错误的原因，用单行注视的方式。
class Demo
{
...
若删除此语句：System.out.println("A");则结果为：B D
...
还有一个奖金属性。请使用继承的思想设计出员工类和经理类。要求
类中提供必要的方法进行属性访问。
...
如果传入的数组为null，应抛出IlleagalArgumentException异常。
在类的mian方法中以各种可能出现的情况测试验证该方法编写的是否正确。
例如，字符不存在，字符存在，传入的数组为null等。

public int getIndex(char[] arr,char key)
{
	if(arr==null)
		throw new IlleagelArgumentException("数组为空");
